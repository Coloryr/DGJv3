...
namespace ExtendNetease_DGJModule
{
    public class ExtendNeteaseModule : SearchModule
    {
        ...
        protected override DGJSongInfo GetById(string id)
        {
            return Task.Factory.StartNew(() =>
            {
                try
                {
                    SongInfo song = NeteaseMusicApis.GetSongDetail(_client, long.Parse(id)).ConfigureAwait(false).GetAwaiter().GetResult();
                    if (song != null)
                    {
